...
void Gost_r::E_f(unsigned char A[], unsigned char K[], unsigned char R[]) 
{ 
...
//ГОСТ 28147-89
void Gost_r::E(unsigned char D[], unsigned char K[], unsigned char R[]) 
{ 
    Block32 A, B;                               
...
//Преобразование A(Y)
void Gost_r::A_Y(unsigned char Y[], unsigned char R[]) 
{
    for (int i = 0; i < 24; i++) 
        R[i] = Y[i + 8];
...
//Преобразование P(Y)
void Gost_r::P_Y(unsigned char Y[], unsigned char R[]) 
{
    for (int i = 0; i < 32; i++) 
        R[i] = Y[fi(i)];
}
//Вычисление PSI(Y)
void Gost_r::psi_Y(unsigned char arr[]) 
{
...
void Gost_r::psi_round(unsigned char arr[], int p) 
{
    while (p--) 
        psi_Y(arr);
}

void Gost_r::f(unsigned char H[], unsigned char M[], unsigned char newH[]) 
{
    Block C[4];
